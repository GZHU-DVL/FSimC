...
def x_u_split(save_path, labels, L, n_class=100, n_each_class=500, 
    ...
    if flag_mismatch:
        ...
        if long_tail_gamma == 1 or long_tail_gamma == 0:  
            ...
            # print(samply_dist_u)
            # print(samply_dist)
            # samply_dist = get_matched_lt_samples(long_tail_gamma, n_each_class, n_class, long_tail_label_ratio)
    ...
    with open(file_path_dist, mode='a') as fff:
        info = '\nmismatch: {}, N0: {}, gamma: {}'.format('bda','none' if n_labeled_class_max==0 else str(n_labeled_class_max),str(1) if long_tail_gamma==0 else str(long_tail_gamma))
        fff.write("="*100)
        fff.write(info)
        fff.write('\nLB: ')
