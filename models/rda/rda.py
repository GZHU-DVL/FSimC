...
class RDA:
    i = 1
    bank = torch.tensor([[]])
    # T = torch.tensor(0.01).cuda()
    # Tc = torch.tensor([0.01, 0.01, 0.01, 0.01, 0.01, 0.01, 0.01, 0.01, 0.01, 0.01,
    ...
    #                   0.01, 0.01, 0.01, 0.01, 0.01, 0.01, 0.01, 0.01, 0.01, 0.01,
    #                   0.01, 0.01, 0.01, 0.01, 0.01, 0.01, 0.01, 0.01, 0.01, 0.01,
    #                   0.01, 0.01, 0.01, 0.01, 0.01, 0.01, 0.01, 0.01, 0.01, 0.01]).cuda()
    ...
    @torch.no_grad()
    def _update_bank(self, fun):
        # if RDA.i == 1:
        #     RDA.bank = fun
        #     RDA.i = 2
        # else:
        #     RDA.bank  = 0.999 *RDA.bank + (1-0.999)*fun
        if RDA.i == 1:
            RDA.bank = fun
            RDA.i = 2
        # else:
        #     RDA.bank  = 0.999 *RDA.bank + (1-0.999)*feature[:num_lb]
        #     RDA.i = RDA.i + 1
        elif RDA.bank.shape[0] <= 1024:
            RDA.bank = torch.cat([RDA.bank, fun],0)
        elif RDA.bank.shape[0] > 1024:
            _, RDA.bank = torch.split(RDA.bank, [fun.shape[0], RDA.bank.shape[0] - fun.shape[0]])
            RDA.bank = torch.cat([RDA.bank, fun],0)
...
##############################################
                ...
                # feature_x_ulb_w, feature_x_ulb_s = feature[num_lb:].chunk(2)
                # if RDA.i == 1:
                #     bank0 = feature_x_ulb_w
                #     bank1 = feature_x_ulb_s
                #     RDA.i=RDA.i+1
                # elif bank0.shape[0]<=2048:
                #     bank0 = torch.cat((bank0,feature_x_ulb_w),dim=0)
                #     bank1 = torch.cat((bank1, feature_x_ulb_s),dim=0)
                # elif bank0.shape[0]>2048:
                #     _, bank0 = torch.split(bank0, [feature_x_ulb_w.shape[0], bank0.shape[0] - feature_x_ulb_w.shape[0]])
                #     _, bank1 = torch.split(bank1, [feature_x_ulb_s.shape[0], bank1.shape[0] - feature_x_ulb_s.shape[0]])
                #     bank0 = torch.cat((bank0,feature_x_ulb_w),dim=0)
                #     bank1 = torch.cat((bank1, feature_x_ulb_s),dim=0)
                #
                #
                # teacher_prob_orig = feature_x_ulb_w @ bank0.t()
                # teacher_prob_orig = F.softmax(teacher_prob_orig / 0.1, dim=1)
                #
                # student_logits = feature_x_ulb_s @ bank1.t()
                # student_prob = F.softmax(student_logits / 0.1, dim=1)
                #
                # teacher_prob_orig = torch.sigmoid(teacher_prob_orig)
                # student_prob = torch.sigmoid(student_prob)
                # loss_in = -torch.sum(teacher_prob_orig.detach() * torch.log(student_prob), dim=1)
                # loss_in = loss_in.mean()


                # if RDA.i == 1:
                # RDA.bank = feature[:num_lb]
                #     RDA.i = 2
                ## else:
                ##     RDA.bank  = 0.999 *RDA.bank + (1-0.999)*feature[:num_lb]
                ##     RDA.i = RDA.i + 1
                # elif bank.shape[0] <= 1024:
                #     bank = torch.cat([bank, feature[:num_lb]],0)
                # elif bank.shape[0] >= 1024:
                #     _, bank = torch.split(bank, [num_lb, bank.shape[0] - num_lb])
                #     bank = torch.cat([bank, feature[:num_lb]],0)
                self._update_bank(feature[:num_lb])

                feature_x_ulb_w, feature_ulb_s = feature[num_lb:].chunk(2)

                teacher_prob_orig = feature_x_ulb_w @ RDA.bank .t()
                teacher_prob_orig = F.softmax(teacher_prob_orig / 0.1, dim=1)

                student_logits = feature_ulb_s @ RDA.bank .t()
                student_prob = F.softmax(student_logits / 0.1, dim=1)
                teacher_prob_orig = torch.sigmoid(teacher_prob_orig)
                student_prob = torch.sigmoid(student_prob)
                loss_in = -torch.sum(teacher_prob_orig.detach() * torch.log(student_prob), dim=1)
...
###############################################
                ...
                ##################
                # log_w = torch.softmax(logits_x_ulb_w, dim=-1)
                # maxvalue, maxindex = torch.max(log_w, dim=-1)
                # RDA.T = 0.999*RDA.T + (1-0.999) * maxvalue.mean()
                #
                # RDA.Tc = 0.999*RDA.Tc + (1-0.999)*log_w.mean(0)
                # maxvalue1, maxindex1 = torch.max(RDA.Tc, dim=0)
                # RDA.Tc = (RDA.Tc/maxvalue1)*RDA.T
                #####################
                ...
                total_loss = sup_loss + reverse_loss + self.lambda_u * unsup_loss_cd + self.lambda_u * unsup_loss_ca + loss_in
