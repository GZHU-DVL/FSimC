...
def consistency_loss_rda(logits_x_ulb_w_reverse, logits_x_ulb_s_reverse, logits_w, logits_s, distri, distri_reverse ,name='ce'):
    ...
    if name == 'L2':
        ...
        #
        #distri = torch.tensor(partition_assign(distri.cpu().numpy(), 1), dtype=torch.float).cuda()
        distri_ = torch.ones_like(distri) - distri 
        distri_ = normalize_d(distri_)      
        pseudo_label_reverse_da = pseudo_label_reverse#normalize_d(pseudo_label_reverse * (torch.mean(distri_,dim=0) / torch.mean(distri_reverse,dim=0)))
        #
        #distri_reverse = torch.tensor(partition_assign(distri_reverse.cpu().numpy(),50),dtype=torch.float).cuda()
        distri_reverse_ = torch.ones_like(distri_reverse) - distri_reverse 
        distri_reverse_ = normalize_d(distri_reverse_)

        pseudo_label_da = pseudo_label#normalize_d(pseudo_label * (torch.mean(distri_reverse_,dim=0) / torch.mean(distri,dim=0)))
